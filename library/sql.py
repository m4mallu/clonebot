...
async def del_from_to_ids(id):
    with INSERTION_LOCK:
        try:
            SESSION.query(Clonebot).filter(Clonebot.id == id).update(
                {"from_id": 0, "to_id": 0, "last_msg_id": 0}
            )
        finally:
            SESSION.commit()
    ...
    with INSERTION_LOCK:
        try:
            ...
            if status is True:
                ...
                SESSION.query(Clonebot).filter(Clonebot.id == id).update(
                    {"caption": True, "file_caption": False}
                )
        finally:
            SESSION.commit()
    ...
    with INSERTION_LOCK:
        try:
            ...
            if status is True:
                ...
                SESSION.query(Clonebot).filter(Clonebot.id == id).update(
                    {"file_caption": True, "caption": False}
                )
        finally:
            SESSION.commit()
